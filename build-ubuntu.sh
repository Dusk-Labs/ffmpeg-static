...
WORK_DIR="$ENV_ROOT/sources"
DEST_DIR="$ENV_ROOT/target"
...
installLibs() {
    ...
    . /etc/os-release
    case "$ID" in
        pop | ubuntu | linuxmint ) installAptLibs ;;
        * )                  installYumLibs ;;
    esac
}
...
installCUDASDKdeb2004() {
    local CUDA_REPO_URL="https://developer.download.nvidia.com/compute/cuda/11.4.2/local_installers/cuda-repo-ubuntu2004-11-4-local_11.4.2-470.57.02-1_amd64.deb"
    Wget "$CUDA_REPO_URL"
    sudo dpkg -i "$(basename "$CUDA_REPO_URL")"
    sudo apt-key adv --fetch-keys "$CUDA_REPO_KEY"
    sudo apt-get -y update
...
installCUDASDK() {
    ...
    case "$ID-$VERSION_ID" in
        ...
        ubuntu-20.04 | ubuntu-21.04 | pop-21.04 ) installCUDASDKdeb2004 ;;
